...
@section('content')
    <div class="container-fluid p-0"> <!-- Ganti container dengan container-fluid -->
        <!-- Bootstrap Carousel -->
                ...
                <tbody>
                    @foreach ($outlets as $index => $outlet)
                        <tr>
                            ...
                            <td>
                                <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', $outlet->phone_number) }}"
                                    target="_blank">
                                    {{ $outlet->phone_number }}
                                </a>
                            </td>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            outlets.forEach(function(outlet) {
                L.marker([outlet.latitude, outlet.longitude])
                    .addTo(map)
                    .bindPopup(
                        "<b>{{ $outlet->name }}</b><br>{{ $outlet->address }}<br><a href='https://wa.me/{{ preg_replace('/[^0-9]/', '', $outlet->phone_number) }}' target='_blank'>Contact via WhatsApp</a>"
                        )
                    .openPopup();
                (`
                    <b>${outlet.name}</b><br>
